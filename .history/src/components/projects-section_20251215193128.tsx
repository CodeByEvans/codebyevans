...
const Carousel = ({ radius = 1.4, count = 3, progress = 0 }) => {
  return Array.from({ length: count }, (_, i) => (
    <Card
      key={i}
      url={images[i]}
      position={[
        Math.sin((i / count) * Math.PI * 2 + progress * Math.PI * 2) * radius,
        0,
        Math.cos((i / count) * Math.PI * 2 + progress * Math.PI * 2) * radius,
      ]}
      rotation={[
        0,
        Math.PI + (i / count) * Math.PI * 2 + progress * Math.PI * 2,
        0,
      ]}
    />
  ));
...
export const ProjectsSection = () => {
  ...
  // Trackea scroll relativo a la sección
  useEffect(() => {
    const handleScroll = () => {
      if (!containerRef.current) return;
      const rect = containerRef.current.getBoundingClientRect();
      const windowHeight = window.innerHeight;
      // progreso relativo: 0 = empieza sección, 1 = termina sección
      const p = Math.min(
  ...
  // Marca cuando la vuelta completa ha terminado
  useEffect(() => {
    setFinished(progress >= 1);
  }, [progress]);

  return (
    <section ref={containerRef} className="w-full h-screen">
      <h1>Vuelta completa: {finished ? "Sí" : "No"}</h1>
