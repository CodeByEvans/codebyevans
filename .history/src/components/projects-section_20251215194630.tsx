...
const Rig = (props: RigProps) => {
  ...
  useFrame((state, delta) => {
    if (!ref.current) return;
    const invertedScroll = 1 - scroll.offset;
    ref.current.rotation.y = invertedScroll * Math.PI * 2; // Rotate contents
...
export const ProjectsSection = () => {
  ...
  return (
    <section id="proyectos" ref={containerRef} className="w-full h-screen">
      <h1>
        {isVisible
      ...
      <Canvas camera={{ position: [0, 0, 100], fov: 22 }}>
        ...
        <ScrollControls pages={3} enabled>
