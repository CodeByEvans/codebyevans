...
const Rig = (props: RigProps) => {
  ...
  useFrame((state, delta) => {
    if (!ref.current) return;
    ref.current.rotation.y = -props.scrollRatio * (Math.PI * 2); // Rotate contents
...
const Banner = (props: BannerProps) => {
  ...
  // Referencia para la malla (opcional, pero buena práctica)
  ...
  useFrame((state, delta) => {
    ...
    // Accedemos a las propiedades a través de la referencia del material
  ...
  return (
    <mesh ref={meshRef} {...props}>
      ...
      <meshSineMaterial
        // Asignar la referencia al material
...
const getScrollRatio = () => {
  ...
  return docHeight > 0 ? Math.min(1, scrollY / docHeight) : 0;
};

export const ProjectsSection = () => {
  const [isVisible, setIsVisible] = useState(false);
  const containerRef = useRef(null);
  const [scrollRatio, setScrollRatio] = useState(getScrollRatio());
  ...
  useEffect(() => {
    ...
    // 2. Logica para el scroll del navegador
