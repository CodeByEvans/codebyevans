...
const Carousel = ({ radius = 1.4, count = 3, scrollProgress = 0 }) => {
  return Array.from({ length: count }, (_, i) => (
    <Card
      ...
      rotation={[0, Math.PI + (i / count) * Math.PI * 2 + scrollProgress, 0]}
    />
  ));
};
...
export const ProjectsSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = 1 - rect.top / window.innerHeight; // 0 -> arriba, 1 -> abajo
